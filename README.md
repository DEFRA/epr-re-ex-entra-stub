...
> [!WARNING]
> The `epr-re-ex-entra-stub` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `epr-re-ex-entra-stub`. To turn it
...
```bash
docker build --target development --no-cache --tag epr-re-ex-entra-stub:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 epr-re-ex-entra-stub:development
```
...
```bash
docker build --no-cache --tag epr-re-ex-entra-stub .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 epr-re-ex-entra-stub
```
...
```bash
docker run -it --entrypoint /bin/ash epr-re-ex-entra-stub
```
